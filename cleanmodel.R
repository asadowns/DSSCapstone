...
if (!file.exists(dirname)) {
  ...
  close(twitterFile)
  sampledTwitter <- sample(twitter,length(twitter)*percent)
  output<-file(paste0(dirname,"/twitter.txt"))
  ...
  close(blogsFile)
  sampledblogs <- sample(blogs,length(blogs)*percent)
  output<-file(paste0(dirname,"/blogs.txt"))
  ...
  close(newsFile)
  samplednews <- sample(news,length(news)*percent)
  output<-file(paste0(dirname,"/news.txt"))
...
source <- DirSource(dirname) #input path for documents
dbName <- paste0(dirname,"corpus")
if(!file.exists(dbName)){
  ...
  #stopwordsProfane <- tolower(c(stopwords('en'),stopwordsProfane))
  ...
  files <- dbLoad(db)
  corpus <- as.VCorpus(dbMultiFetch(db,files))
}
...
predictNgram <- function(sentence) {
  ...
  rmStopwords <- processed %>% 
    removeWords(stopwords("english")) %>%
    stripWhitespace() %>%
    trim()
  
  reducePattern <- str_replace_all(rmStopwords, "\\s+","|")
  
  values <- tm_index(corpus, FUN=function(x) grep(reducePattern, content(x),value=TRUE))
  values <- as.matrix(values)
  values <- VCorpus(VectorSource(values))
  
#  for (i in 2:(pmaxLength+1)) {
#    dtm <- DocumentTermMatrix(values, list(tokenize = function(x) NgramTokenizer(x,i)))
...
#  }
  ...
  tokenLength <- strlength + 1
  pattern <- paste(rep("\\w+\\s+",strlength-1),collapse = "",sep="")
  pattern <- paste0(pattern,"\\w+$")
  predictor <- str_extract(processed,pattern)
  print(predictor)
  dtm <- DocumentTermMatrix(values, list(tokenize = function(x) NgramTokenizer(x,tokenLength)))
  gram <- sort(col_sums(dtm), decreasing=TRUE)
